...
<a href = "#" style = "color:#5d5c5c; ; text-decoration: none;"> <?php echo $category ? htmlspecialchars($category) : 'Smart devices'; ?></a> 
</p>
...
<div class="filters-row">
    <div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap;">
        <label style = "font-size: 15px;"> Sort by
            <select onchange="window.location.href='?<?php echo buildQueryString(['sort' => '']); ?>&sort='+this.value" style="padding: 5px 10px; border-radius: 6px;">
...
<!-- Slightly Wider Yellow Filter Box (No Price Range) -->
<aside class = "filter-panel" style="background-color: #fff8e1; border: 2px solid #ffd54f; border-radius: 10px; padding: 18px; height: fit-content;">
    <div style="font-size: 17px; font-weight: bold; margin-bottom: 15px; color: #333; text-align: center;">FILTERS</div>

    <div class = "filter-group" style="margin-bottom: 20px;">
        <div style="font-size: 15px; font-weight: 600; margin-bottom: 10px; color: #444; padding-bottom: 5px; border-bottom: 1px solid #ffd54f;">CATEGORIES</div>
        <div style="display: flex; flex-direction: column; gap: 8px;">
            <label style="display: flex; align-items: center; gap: 8px; font-size: 14px; padding: 6px 8px; border-radius: 5px; transition: background 0.2s;" 
                   onmouseover="this.style.background='#ffedb3'" onmouseout="this.style.background='transparent'">
                <input type="radio" name="db_category" value="" <?php echo !$category ? 'checked' : ''; ?> onchange="window.location='products.php'" style="width: 16px; height: 16px;">
                <span>All Products</span>
            </label>
            <?php foreach($categories as $cat): ?>
            <label style="display: flex; align-items: center; gap: 8px; font-size: 14px; padding: 6px 8px; border-radius: 5px; transition: background 0.2s;"
                   onmouseover="this.style.background='#ffedb3'" onmouseout="this.style.background='transparent'">
                <input type="radio" name="db_category" value="<?php echo htmlspecialchars($cat); ?>" <?php echo $category == $cat ? 'checked' : ''; ?> onchange="window.location='products.php?category=<?php echo urlencode($cat); ?>'" style="width: 16px; height: 16px;">
    ...
    <div class="filter-group" style="margin-bottom: 20px;">
        <div style="font-size: 15px; font-weight: 600; margin-bottom: 10px; color: #444; padding-bottom: 5px; border-bottom: 1px solid #ffd54f;">BRANDS</div>
        <div style="display: flex; flex-direction: column; gap: 8px;">
            <?php foreach($brands as $b): ?>
            <label style="display: flex; align-items: center; gap: 8px; font-size: 14px; padding: 6px 8px; border-radius: 5px; transition: background 0.2s;"
                   onmouseover="this.style.background='#ffedb3'" onmouseout="this.style.background='transparent'">
                <input type="checkbox" name="brand_check" value="<?php echo htmlspecialchars($b); ?>" <?php echo $brand == $b ? 'checked' : ''; ?> onchange="window.location='products.php?brand=<?php echo urlencode($b); ?>'" style="width: 16px; height: 16px;">
    ...
    <div style="margin-top: 20px; padding-top: 15px; border-top: 1px solid #ffd54f;">
        <a href="products.php" style="display: block; text-align: center; padding: 10px; background: #ffee32; border-radius: 6px; color: #111; font-weight: bold; text-decoration: none; font-size: 14px; transition: background 0.2s;" 
...
<div class="product-results">
    <div class="result-meta">
        ...
        <div style="display: flex; gap: 10px; align-items: center;">
            <div class="products-per-page" style="font-size: 14px; color: #666;">
                <span>Show: </span>
                <select onchange="window.location.href='?<?php echo buildQueryString(['per_page' => '']); ?>&per_page='+this.value" style="padding:5px 10px; border-radius:4px; font-size: 14px; border: 1px solid #ddd;">
            ...
            <a href="products.php" style = "text-decoration:none; color: #111; font-weight: bold; font-size: 14px; padding: 5px 10px; border-radius: 4px; background: #f5f5f5;">Reset filters</a>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 22px; margin-top: 20px;">
        <?php if(mysqli_num_rows($result) > 0): ?>
            ...
            <div class="product-card" style="background: white; border-radius: 12px; padding: 18px; border: 1px solid #eee; box-shadow: 0 3px 10px rgba(0,0,0,0.05); display: flex; flex-direction: column; transition: transform 0.2s, box-shadow 0.2s;" 
                ...
                <div class="product-image" style="background: #f9f9f9; border-radius: 10px; padding: 12px; margin-bottom: 12px; text-align: center; min-height: 180px; display: flex; align-items: center; justify-content: center;">
                ...
                <h3 style="margin: 8px 0; font-size: 16px; font-weight: bold; color: #333; min-height: 40px; line-height: 1.3;"><?php echo htmlspecialchars($product['product_name']); ?></h3>
                ...
                <div style="margin: 10px 0; color: #666; font-size: 13px; line-height: 1.4; min-height: 40px; flex-grow: 1;">
                    <?php if(!empty($points)): ?>
                        <?php foreach($points as $point): ?>
                        <div style="margin-bottom: 4px; font-size: 12px;">• <?php echo htmlspecialchars(trim($point)); ?></div>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <div style="font-size: 12px;"><?php echo strlen($description) > 80 ? substr(htmlspecialchars($description), 0, 80) . '...' : htmlspecialchars($description); ?></div>
                    <?php endif; ?>
                </div>
                ...
                <div style="display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 12px;">
                    <?php if($product['stock_quantity'] > 0): ?>
                        <span style="background: #2bb334; color: white; padding: 3px 8px; border-radius: 10px; font-size: 11px; font-weight: bold;">
                    ...
                    <?php else: ?>
                        <span style="background: #ff4444; color: white; padding: 3px 8px; border-radius: 10px; font-size: 11px; font-weight: bold;">
                            Out of Stock
                        </span>
                    <?php endif; ?>
                    
                    <?php if($product['price'] > 50): ?>
                    <span style="background: #111; color: white; padding: 3px 8px; border-radius: 10px; font-size: 11px; font-weight: bold;">
                        Free Delivery
                    </span>
                    <?php endif; ?>
                </div>
                ...
                <div class="actions" style="display: flex; gap: 8px; margin-top: auto;">
                    ...
                    <?php if(isLoggedIn()): ?>
                        ...
                        <?php else: ?>
                        <button disabled style="flex: 1; padding: 10px; background: #ccc; border: none; border-radius: 6px; color: #666; font-weight: bold; font-size: 14px; cursor: not-allowed;">
                            Out of Stock
                        </button>
                        <?php endif; ?>
                    <?php else: ?>
            ...
            <?php endwhile; ?>
        <?php else: ?>
            <div style="grid-column: 1 / -1; text-align: center; padding: 50px 20px; background: white; border-radius: 12px; border: 1px solid #eee; box-shadow: 0 3px 10px rgba(0,0,0,0.05);">
                <h2 style="color: #666; margin-bottom: 15px; font-size: 20px;">No products found</h2>
                <p style="color: #888; margin-bottom: 25px; max-width: 500px; margin-left: auto; margin-right: auto; font-size: 14px;">
                    <?php echo $search ? 'No results for "' . htmlspecialchars($search) . '"' : 'Try changing your filters'; ?>
                </p>
                <a href="products.php" style="text-decoration:none;display:inline-block;padding:12px 30px; background: #ffee32; border-radius: 6px; color: #111; font-weight: bold; font-size: 14px; transition: background 0.2s;" 
                   onmouseover="this.style.background='#e0d129'" onmouseout="this.style.background='#ffee32'">
                    View all products
                </a>
            </div>
    ...
    <div style="display:flex;justify-content:center;gap:8px;margin-top:40px;padding-top:25px;border-top:1px solid #eee;">
        <?php if($page > 1): ?>
        ...
        <?php for($i = 1; $i <= $total_pages; $i++): ?>
            ...
            <a href="?<?php echo buildQueryString(['page' => $i]); ?>" 
               ...
               onmouseover="this.style.background='<?php echo $i == $page ? '#e0d129' : '#f5f5f5'; ?>'" onmouseout="this.style.background='<?php echo $i == $page ? '#ffee32' : 'transparent'; ?>'">
                <?php echo $i; ?>
            </a>
